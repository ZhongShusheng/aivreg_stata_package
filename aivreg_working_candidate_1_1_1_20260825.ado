*! aivreg 1.1.1 25aug2026 (SSC release candidate; lean sprint AIVREG-RC-004; from 03452fa4)
...
foreach v of local varlist {
	...
	if `L' > 1 {
		...
		* Valid ONLY on the efficient two-step GMM path; one-step/2sls stay suppressed.
		if "`weightmatrix'" != "" & "`2sls'" != "2sls" {
			...
			* DESIGN_SPEC_v1 (Gate-1 frozen) §B: J df is RANK-based over-identification,
			* rank(S_first) - rank(params), not the moment-dimension count.
			* rank(W2) = rank(invsym(S_first)) = rank(S_first); params rank from XtX.
			mata: st_numscalar("__aivreg_rk_W2",  rank(st_matrix("weightmatrix")))
			mata: st_numscalar("__aivreg_rk_par", rank(st_matrix("XtX")))
			local Jdof = __aivreg_rk_W2 - __aivreg_rk_par
			...
			if `Jdof' >= 1 {
				...
				* rank-deficient over-identification: no valid chi2 reference
	...
	****************************************************************************
	...
	if `L' > 1 {
		...
		if `twostep_efficient' == 1 {
			...
			display as text "J-stat: not reported -- rank-based over-identification df < 1 on the efficient two-step path (rank-deficient moment covariance)."
		}
		else {
	...
	if `L' > 1 {
		...
		* never emit invalid p until efficient two-step J passes QA.
		if `twostep_efficient' == 1 {
			...
			* Rank-deficient over-identification on the efficient two-step path:
			* rank-based df < 1 gives no valid chi2 reference (DESIGN_SPEC_v1 §B).
			ereturn scalar Jval = .
			ereturn scalar pval_J = .
			ereturn local J_status "SUPPRESSED_twostep_rank_deficient_overid_AIVREG_RC004"
		}
		else {
	...
	* AIVREG-8 e(sample) survivor remap: snapshot analytic survivors, restore full data, mark sample
	tempfile _survf
	quietly keep `_srcobs'
	quietly duplicates drop `_srcobs', force
	quietly save `_survf', replace
	restore
	capture noisily merge 1:1 `_srcobs' using `_survf', keep(master match) gen(_esm)
	if _rc == 0 {
		tempvar _es
		quietly gen byte `_es' = (_esm == 3)
		quietly drop _esm
		capture ereturn repost, esample(`_es')
		if _rc == 0  ereturn local esample_status "REMAPPED_AIVREG8"
		else         ereturn local esample_status "REPOST_FAILED"
	}
	else ereturn local esample_status "MERGE_FAILED"
end
